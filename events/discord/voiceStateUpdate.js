...
module.exports = async (client, oldMember, newMember) => {
  ...
  if (oldMember.channelID === null) {
    if (newMember.channelID === '822310255324299294') return client.channels.cache.get('825674456470519809').send(`${newMember.member.user.tag}が${newMember.channel.name}に入室しました`);
    ...
    if (oldMember.channelID === '822310255324299294') return client.channels.cache.get('825674456470519809').send(`${oldMember.member.user.tag}が${oldMember.channel.name}から退出しました`);
    ...
    if (oldMember.channelID === '822310255324299294') {
      client.channels.cache.get('706458716320432198').send(`${newMember.member.user.tag}が${newMember.channel.name}に入室しました`);
      return client.channels.cache.get('825674456470519809').send(`${oldMember.member.user.tag}が${oldMember.channel.name}から退出しました`);
    }
