...
module.exports = {
    ...
    run: async function (client, message, args) {
        try {
            if (message.channel.parentId !== '821684794056245258' && !message.channel.isThread()) return message.reply({ content: 'そのコマンドはお問い合わせカテゴリーでのみ使用できます', allowedMentions: { repliedUser: false } }).catch(error => errorlog(message, error));
            if (message.channel.parentId === '821684794056245258') {
                ...
                client.channels.cache.get('870145872762126437').threads.cache.get(message.channel.id).setArchived(true, `Archived By ${message.author.tag}`);
            }
        } catch (error) {
