    ...
    run: async function (client, message, args) {
        try {
            if (!message.member.roles.cache.has('822852335322923060') && !message.member.roles.cache.has('771015602180587571')) {
                ...
                if (!args[0]) return await message.reply('第一引数にあなたのゲーマータグを入れてください！');
                const data = {
                ...
                await message.reply('あなたのゲーマータグをDiscordアカウントとリンクしました！');
            }
            else {
                ...
                if (!user) {
                    ...
                    if (!args[1]) return await message.reply(`第二引数に${user.tag}のゲーマータグを入れてください！`);
                    const data = {
                        id: `${user.id}-${args[1]}`,
                        user: user.id,
                        tag: args[1]
                    ...
                    client.db.prepare('INSERT INTO gamertags (id, user, tag) VALUES (@id, @user, @tag);').run(data);
                    await message.channel.send(`${user.tag}のゲーマータグをDiscordアカウントとリンクしました！`);
                }
            }
        } catch (error) {
